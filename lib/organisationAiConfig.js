...
function orgAiToRuntimeOverrides(orgAi) {
  ...
  if (provider === 'anthropic') {
    const reasoningEnabled =
      typeof orgAi.reasoningEnabled === 'boolean' ? orgAi.reasoningEnabled : false;
    let level = typeof orgAi.reasoningLevel === 'string' ? orgAi.reasoningLevel : 'standard';
    if (!['minimal', 'standard', 'extensive'].includes(level)) {
      level = 'standard';
    }
    if (reasoningEnabled) {
      let budget;
      ...
      overrides.anthropicThinkingBudget = budget;
    }
  }
...
function validateOrgAiUpdate(body) {
  ...
  const out = {
    ...
    reasoningLevel: (function () {
      const raw = String(body.reasoningLevel == null ? '' : body.reasoningLevel).trim();
      if (!raw) return undefined;
      const v = raw.toLowerCase();
      return ['minimal', 'standard', 'extensive'].includes(v) ? v : 'standard';
    })(),
  };
  if (hasNewKey) {
...
function sanitizeOrgAiForProvider(doc) {
  ...
  if (p !== 'anthropic') {
    ...
    if (typeof next.reasoningEnabled !== 'boolean') {
      next.reasoningEnabled = false;
    }
    if (!['minimal', 'standard', 'extensive'].includes(next.reasoningLevel)) {
      next.reasoningLevel = 'standard';
    }
  }
...
function maskOrgAiForClient(orgAi) {
  ...
  return {
    ...
    reasoningEnabled: !!orgAi.reasoningEnabled,
    reasoningLevel:
      typeof orgAi.reasoningLevel === 'string' && orgAi.reasoningLevel.trim()
        ? orgAi.reasoningLevel
        : 'standard',
  };
}
