 ...
 */
function contributorEmailsFromProject(project, owner) {
    const emails = new Set();
    if (owner && owner.email) {
        emails.add(String(owner.email).trim());
    }
    if (project && project.createdBy) {
        emails.add(String(project.createdBy).trim());
    }
    if (project && project.lastModifiedBy) {
        emails.add(String(project.lastModifiedBy).trim());
    }
    const items = Array.isArray(project && project.unintendedConsequences)
        ? project.unintendedConsequences
        : [];
    items.forEach((uc) => {
        if (!uc) return;
        if (uc.createdBy) emails.add(String(uc.createdBy).trim());
        if (uc.lastModifiedBy) emails.add(String(uc.lastModifiedBy).trim());
        const a = uc.action || {};
        if (a.createdBy) emails.add(String(a.createdBy).trim());
        if (a.lastModifiedBy) emails.add(String(a.lastModifiedBy).trim());
        if (a.completedBy) emails.add(String(a.completedBy).trim());
    });
    return Array.from(emails).filter((e) => e);
}

function buildContributorsChildren(project, owner, accentHex) {
    const emails = contributorEmailsFromProject(project, owner);
    if (!emails.length) {
        return [new docx.Paragraph("")];
    }
    const labels = emails
        .map((email) => {
            ...
            if (owner && owner.email && String(owner.email).trim().toLowerCase() === e.toLowerCase()) {
...
function buildAiAssistedNoticeChildren(project, owner, includeProvenanceAppendix, accentHex) {
    const children = [];
    ...
    buildContributorsChildren(project, owner, accentHex).forEach((p) => children.push(p));
    return children.length ? children : [new docx.Paragraph("")];
}
...
async function buildRiskBandSplitImage(riskCounts) {
    try {
        const counts = riskCounts || {};
        const labels = ["High", "Medium", "Low", "Unclassified"];
        const values = [
            Number(counts.high || 0),
            Number(counts.medium || 0),
            Number(counts.low || 0),
            Number(counts.unclassified || 0),
        ];
        ...
        console.error("Error generating risk-band split chart:", error);
        return getFallbackImageBuffer();
    }
}

function buildMatrixCountsFromProject(project) {
    const counts = {
        High: { High: 0, Medium: 0, Low: 0 },
        Medium: { High: 0, Medium: 0, Low: 0 },
        Low: { High: 0, Medium: 0, Low: 0 },
    };
    const risks = Array.isArray(project && project.unintendedConsequences)
        ? project.unintendedConsequences
        : [];
    risks.forEach((risk) => {
        if (!risk || risk.outcome === "Positive") {
            return;
        }
        const impact = String(risk.impact || "");
        const likelihood = String(risk.likelihood || "");
        if (!counts[impact] || typeof counts[impact][likelihood] === "undefined") {
            return;
        }
...
function getMatrixCellFill(impact, likelihood) {
    if (impact === "High" && likelihood === "High") return "F8D7DA";
    if ((impact === "High" && likelihood === "Medium") || (impact === "Medium" && likelihood === "High")) return "FAD8CB";
    if (
        (impact === "Medium" && likelihood === "Medium") ||
        (impact === "High" && likelihood === "Low") ||
        (impact === "Low" && likelihood === "High")
    ) return "FFF3CD";
    if (impact === "Low" && likelihood === "Low") return "D4EDDA";
    if ((impact === "Medium" && likelihood === "Low") || (impact === "Low" && likelihood === "Medium")) return "FAFFDC";
    return "FFFFFF";
}

function buildCountMatrixTable(project, accentHex) {
    const levels = ["High", "Medium", "Low"];
    const counts = buildMatrixCountsFromProject(project);
    const headerFill = accentTableHeaderShading(accentHex);
    return new docx.Table({
        width: 0,
        columnWidths: [1800, 1600, 1600, 1600],
        rows: [
            new docx.TableRow({
                children: [
                    new docx.TableCell({ children: [accentTableHeaderParagraph("Likelihood / Impact")], shading: headerFill }),
                    ...levels.map((l) => new docx.TableCell({ children: [accentTableHeaderParagraph(l)], shading: headerFill })),
                ],
            }),
            ...levels.map((impact) => new docx.TableRow({
                children: [
                    new docx.TableCell({ children: [new docx.Paragraph({ text: impact, bold: true })] }),
                    ...levels.map((likelihood) => new docx.TableCell({
                        shading: { fill: getMatrixCellFill(impact, likelihood) },
                        children: [new docx.Paragraph({ text: String(counts[impact][likelihood] || 0), alignment: docx.AlignmentType.CENTER })],
                    })),
                ],
            })),
        ],
...
function buildRiskCountsSummaryChildren(metrics, accentHex) {
    const counts = (metrics && metrics.riskCounts) || {};
    const rows = [
    ...
    return [
        new docx.Paragraph({
            ...
            children: [new docx.TextRun({ text: "Counts", bold: true, color: accentHex })],
        }),
        ...rows.map(([label, value]) => new docx.Paragraph({
            spacing: { after: 60 },
            children: [
...
function buildRiskEntryMatrixChildren(project, accentHex) {
    const levels = ["High", "Medium", "Low"];
    const entries = {
    ...
    (Array.isArray(project && project.unintendedConsequences) ? project.unintendedConsequences : []).forEach((risk) => {
        if (!risk || risk.outcome === "Positive") return;
        const impact = String(risk.impact || "");
        const likelihood = String(risk.likelihood || "");
        if (!entries[impact] || !entries[impact][likelihood]) return;
        const text = String(risk.consequence || "").trim();
        if (text) entries[impact][likelihood].push(text);
    });
    const headerFill = accentTableHeaderShading(accentHex);
    return [
        new docx.Paragraph({
            spacing: { before: 200, after: 120 },
            children: [new docx.TextRun({ text: "Risk matrix entries", bold: true, color: accentHex })],
        }),
        new docx.Table({
            width: 0,
            columnWidths: [1800, 2300, 2300, 2300],
            rows: [
                new docx.TableRow({
                    children: [
                        new docx.TableCell({ children: [accentTableHeaderParagraph("Likelihood / Impact")], shading: headerFill }),
                ...
                ...levels.map((impact) => new docx.TableRow({
                    children: [
                        ...
                        ...levels.map((likelihood) => new docx.TableCell({
                            shading: { fill: getMatrixCellFill(impact, likelihood) },
                            children: (entries[impact][likelihood].length
                                ? entries[impact][likelihood]
                                : ["—"]
...
async function buildDocx(project, metrics, owner, reportOptions = {}) {
    ...
    let riskBandSplitChartData;
    try {
        riskBandSplitChartData = await buildRiskBandSplitImage(metrics.riskCounts);
    } catch (error) {
        console.error("Failed to generate report charts, using fallback:", error);
        riskBandSplitChartData = getFallbackImageBuffer();
    }
        ...
        const doc = await docx.patchDocument(templateBuffer, {
            ...
            patches: {
                ...
                aiAssistedNotice: {
                    ...
                    children: buildAiAssistedNoticeChildren(project, owner, includeAiProvenance, accentHex),
                },
                ...
                riskMatrixChart: {
                    type: docx.PatchType.DOCUMENT,
                    children: [buildCountMatrixTable(project, accentHex)],
                },
                riskCountsSummary: {
                    type: docx.PatchType.DOCUMENT,
                    children: buildRiskCountsSummaryChildren(metrics, accentHex),
                },
                riskEntryMatrix: {
                    type: docx.PatchType.DOCUMENT,
                    children: buildRiskEntryMatrixChildren(project, accentHex),
                },
                topRisks: {
                    type: docx.PatchType.DOCUMENT,
                    children: [
                                                ...
                                                children: [
                                                    ...
                                                    new docx.Paragraph({
                                                        children: [
                                                            ...
                                                            new docx.TextRun(
                                                                unintendedConsequence.action?.completed
                                                                ? `${unintendedConsequence.action.completed}`
                                                                : 'Not recorded'
                                                            )
                                                        ]
                                                    ...
                                                    new docx.Paragraph({
                                                        children: [
                                                            ...
                                                            new docx.TextRun(
                                                                unintendedConsequence.action?.completedAt
                                                                ? `${unintendedConsequence.action.completedAt}`
                                                                : 'N/A'
                                                            )
                                                        ]
                                                    ...
                                                    new docx.Paragraph({
                                                        children: [
                                                            ...
                                                            new docx.TextRun(
                                                                unintendedConsequence.action?.completionComment
                                                                ? `${unintendedConsequence.action.completionComment}`
                                                                : 'N/A'
                                                            )
                                                        ]
