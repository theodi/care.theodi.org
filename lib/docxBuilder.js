...
async function buildDocx(project,metrics, owner) {
    const positiveUnintended = project.unintendedConsequences.filter(u => u.outcome === 'Positive');
    const negativeUnintended = project.unintendedConsequences.filter(u => u.outcome !== 'Positive');
