...
/** Table header cell shading (unchanged from main report tables). */
const TEMPLATE_BLUE_SHADING = { fill: "072589", color: "ffffff" };
/** Template navy for non–TOC labels (glossary letters, AI section titles). */
const TEMPLATE_BLUE_TEXT = "072589";
...
function buildGlossaryAppendixChildren(include, glossary) {
    if (!include) {
...
function pushLabelParagraph(children, text) {
    children.push(
        new docx.Paragraph({
...
function buildSuggestionsWithSelectionTable(run) {
    const suggestions = Array.isArray(run.suggestions) ? run.suggestions : [];
    const selected = selectedIndicesFromRun(run);
    const headerRow = new docx.TableRow({
        children: [
            new docx.TableCell({
                children: [new docx.Paragraph("Selected")],
                shading: TEMPLATE_BLUE_SHADING,
            }),
            new docx.TableCell({
 ...
 */
function buildAiAssistedNoticeChildren(project, includeProvenanceAppendix) {
...
function buildAiProvenanceAppendixChildren(include, project) {
    if (!include) {
        return [new docx.Paragraph("")];
    }
    const children = [
    ...
    hist.forEach((run, idx) => {
        ...
        pushLabelParagraph(children, "Prompt (full text sent to the model)");
        pushMonospaceBlock(children, run.promptFull);
        if (run.reasoning && String(run.reasoning).trim()) {
            pushLabelParagraph(children, "AI reasoning");
            pushMonospaceBlock(children, run.reasoning);
        }
        pushLabelParagraph(children, "Model responses");
        children.push(buildSuggestionsWithSelectionTable(run));
    });
    return children;
}
        ...
        const doc = await docx.patchDocument(templateBuffer, {
            ...
            patches: {
                ...
                aiAssistedNotice: {
                    ...
                    children: buildAiAssistedNoticeChildren(project, includeAiProvenance),
                },
                ...
                aiProvenance: {
                    type: docx.PatchType.DOCUMENT,
                    children: buildAiProvenanceAppendixChildren(includeAiProvenance, project),
                },
                glossaryAppendix: {
                    type: docx.PatchType.DOCUMENT,
                    children: buildGlossaryAppendixChildren(includeGlossaryAppendix, glossaryData),
                },
