...
async function checkMongo({ uri, dbName, mongoose, timeoutMs, disconnect }) {
  const name = 'MongoDB';
  if (isBlank(uri)) {
  ...
  // Default: disconnect only if this check opened the connection (safe for CLI).
  // Pass disconnect: false when using the app's shared mongoose connection.
  const shouldDisconnect = disconnect !== false;
  ...
  try {
    ...
    if (shouldDisconnect && openedHere && mongoose && typeof mongoose.disconnect === 'function') {
      try {
 ...
 */
async function runConnectionChecks({ env, mongoose, fetchImpl, loadAiConfig, timeoutMs, disconnect } = {}) {
